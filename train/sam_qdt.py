...
def main(args):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            qimages = torch.reshape(qimages,shape=(-1,3,2*35,2*35))
            qmasks = torch.reshape(qmasks,shape=(-1,1,2*35,2*35))
        ...
        if (epoch + 1) % 3 == 0:  # Adjust the frequency of visualization
            ...
            with torch.no_grad():
                ...
                qsample_images = torch.reshape(qsample_images,shape=(-1,3,2*35,2*35))
                qsample_masks = torch.reshape(qsample_masks,shape=(-1,1,2*35,2*35))
