...
def main(args):
    ...
    model = APT(qdt_shape=(patch_size*16, patch_size*16),
            input_dim=3, 
            output_dim=1, 
            embed_dim=768,
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            qimages = torch.reshape(qimages,shape=(-1,3,patch_size*16, patch_size*16))
            qmasks = torch.reshape(qmasks,shape=(-1,1,patch_size*16, patch_size*16))
        ...
        if (epoch + 1) % 3 == 0:  # Adjust the frequency of visualization
            ...
            with torch.no_grad():
                ...
                qsample_images = torch.reshape(qsample_images,shape=(-1,3,patch_size*16, patch_size*16))
                qsample_masks = torch.reshape(qsample_masks,shape=(-1,1,patch_size*16, patch_size*16))
